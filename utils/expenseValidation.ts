...
export function validateExpense(data: unknown): any[] | null {
...
export function validateExpenseUpdate(data: unknown): any[] | null {
  const parsed = expenseUpdateSchema.safeParse(data);
  if (!parsed.success) return parsed.error.issues;
  return null;
}

export function validateBudget(data: unknown): any[] | null {
